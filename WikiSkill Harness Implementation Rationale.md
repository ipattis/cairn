...
Build a macOS system in three parts: a Rust daemon, `wikiskilld`, that runs WikiSkill; a Tauri desktop cockpit that controls it; and **OpenCode**, pinned to the stable V1 line (1.18.x), as the executor for both rollouts and daily coding. OpenCode is the only harness that meets every must-have through configuration, and Rust fits the parts whose logic is ours.

Why OpenCode stays the executor:

- **All three providers work.** Amazon Bedrock and Fireworks AI are built-in providers. Bedrock Mantle attaches as a custom OpenAI-compatible provider ([providers](https://opencode.ai/docs/providers/)).
- **Its server has an HTTP API.** The daemon talks to it through a Rust client generated from the same OpenAPI spec the JS SDK comes from ([SDK](https://opencode.ai/docs/sdk/)).
- **Skills are native.** OpenCode loads SKILL.md files from `~/.agents/skills`, so the vault's skills folder can be symlinked in ([skills](https://opencode.ai/docs/skills/)).
...
Three caveats shape the design. OpenCode V2 changes the server API, so the daemon reaches it through an `Executor` trait. Bash commands can bypass OpenCode's path rules, so macOS Seatbelt profiles enforce isolation. V1 documents no pre-prompt injection hook, so the Jev skill router waits until a later phase. The runner-up executor is pi.

## Requirements

Thirteen requirements fall out of the spec. Six come from the WikiSkill loop, one from the Obsidian vault, one from Jev, two from the inference providers (Bedrock, Bedrock Mantle, Fireworks), and three from running it as a Mac desktop app. Must-haves decide the harness; should-haves break ties.
...
| R10 | OpenAI-compatible custom providers for Bedrock Mantle and Fireworks | Available inference | Must |
...
| --- | --- | --- | --- | --- | --- |
| [OpenCode](https://opencode.ai/docs/) | Bedrock and Fireworks built in; Mantle as a custom OpenAI-compatible provider | Per-agent rules with path patterns, last match wins; bash can slip past `external_directory` | HTTP server with an OpenAPI spec, callable from Rust; many parallel sessions per server | `session.idle`, `tool.execute.before/after`, message events; no documented pre-prompt hook | **Pick** |
| [pi](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) | Bedrock built in; Fireworks and Mantle via custom provider config (verify) | None by design; build it as an extension | Print/JSON, RPC over stdin/stdout, and an SDK; one process per session | Extension events including `before_agent_start` and `session_shutdown` | Runner-up |
...
**Why not Claude Code, given the best hooks?** WikiSkill found that skills evolved on one model can hurt another, so the loop must evolve and gate on the exact model you deploy. Locking to Claude models removes the Fireworks and Mantle open models from that choice.
...
Rollouts and daily coding run DeepSeek V4 Pro on Fireworks, the model you will code with; V4 Flash takes cheap exploratory rollouts and small tasks. The maintainer and proposer stay on Claude models on Bedrock, and Mantle covers models only it serves. Confirm exact model IDs in the Fireworks catalog and Bedrock console.
...
| Cross-family transfer tests | GPT or open models only on Mantle | Bedrock Mantle, custom provider | Mantle hosts models the runtime endpoint lacks |
| Decision layer | Jev 1.13 | TypeSafe API | A fourth vendor; see the security section |

**Endpoint rule.** AWS recommends `bedrock-runtime` for most new applications, and only it offers cross-Region inference and Guardrails. Use `bedrock-mantle` for Mantle-only models, background inference, or Projects, which let you attribute cost per role. Per-token prices match on both endpoints ([AWS endpoints](https://docs.aws.amazon.com/bedrock/latest/userguide/endpoints.html)).
...
Mantle wiring in `opencode.json` (use `@ai-sdk/openai` instead if a model needs the Responses API):

```json
{
  "model": "fireworks-ai/<deepseek-v4-pro-id>",
  "small_model": "fireworks-ai/<deepseek-v4-flash-id>",
  "provider": {
    "bedrock-mantle": {
      ...
      "models": { "<id-from-GET-/v1/models>": {} }
    }
  }
}
```

The IAM principal needs `bedrock-mantle:CreateInference` for Mantle and `bedrock:InvokeModel` for the runtime endpoint.

**Two client paths.** The OpenCode config above serves rollouts and daily coding. The daemon's curator agents call models directly: Bedrock through the AWS SDK for Rust's Converse API, Mantle and Fireworks through an OpenAI-compatible client with a custom base URL. Keys live in the macOS Keychain and are read only by the daemon.
...
| Mantle and Fireworks | `async-openai` with a custom base URL |
| OpenCode client | generated from its OpenAPI spec with `progenitor` |
...
| --- | --- | --- | --- | --- |
| Inference | OpenCode agent `wiki-inference` on the daemon's `opencode serve` | OpenCode defaults, with `skill` denied | Rollout Seatbelt profile, then OpenCode permissions | Regenerated each iteration: every active SKILL.md concatenated |
...
**Why routing waits.** OpenCode V1 documents no hook that runs before the first model call of a turn, so a router could only inject skills after the model has started. Until then, native lazy skill loading handles daily use, and a Jev Noul per session measures whether a relevant skill was loaded. If that trigger rate is poor, revisit with V2 plugin APIs or pi's `before_agent_start`.
...
Mantle does not support Bedrock Guardrails, so any Guardrails policy must sit on `bedrock-runtime` traffic ([AWS endpoints](https://docs.aws.amazon.com/bedrock/latest/userguide/endpoints.html)).
...
| --- | --- | --- |
| 0. Foundations | Vault and git outside iCloud; `wikiskill-core` with Bedrock, Mantle and Fireworks clients; OpenCode 1.18.x pinned; rollout Seatbelt profile; verifier; 60-task set | Empty-skill validation and test scores on the deploy model; every task's reference solution passes under the profile |
...
| Risk | Mitigation |
| --- | --- |
| OpenCode V2 revises the server API, and V1 integrations must migrate ([migration guide](https://opencode.ai/v2/docs/migrate-v1/)) | Pin 1.18.x; keep every call behind the `Executor` trait; generate the Rust client from the pinned spec |
...
| OpenCode Desktop's Node runtime rejects Bun-only plugin code | Capture plugin uses only the SDK client and `fetch` |
...
- OpenCode docs: [providers](https://opencode.ai/docs/providers/), [permissions](https://opencode.ai/docs/permissions/), [plugins](https://opencode.ai/docs/plugins/), [SDK](https://opencode.ai/docs/sdk/), [skills](https://opencode.ai/docs/skills/), [V2 migration](https://opencode.ai/v2/docs/migrate-v1/)
